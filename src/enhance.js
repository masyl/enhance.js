 ...
 *
 * Modified Date : October 13 2011, 2012
	Enhance.js
...
* Grouped enhancement method by block in the console for better readability
*
 */

(function ($) {
    var hasConsole = typeof console !== "undefined",
		hasConsoleTime = hasConsole && typeof console.time !== "undefined" && $("body").data("enhance-verbose"),
		hasConsoleGroup = hasConsole && typeof console.group !== "undefined" && $("body").data("enhance-verbose"),
        hasConsoleGroupCollapsed = hasConsole && typeof console.groupCollapsed !== "undefined" && $("body").data("enhance-verbose"),
		enhancements = [],
		enhancementsById = {},
    ...
    function timeConsoleEnd(id, desc, obj, target) {
        if (hasConsoleTime) {
            console.timeEnd(desc, obj, target);
        } /*else{
			benchmark(id);
		}*/
    }
    function applyEnhancements() {
        ...
        if (hasConsoleGroup) console.group("Enhance");
        $.each(enhs, function () {

            var id = (this.id !== null) ? "#" + this.id + ": " : "",
				desc = "Enhanced: " + id + this.title;

            timeConsole(this.id, desc);

            if (hasConsoleGroupCollapsed) console.groupCollapsed(this.id + ' enhance logs');
            else if (hasConsoleGroup) console.group(this.id + ' enhance logs');

            //try {
				$(this.elems).data("enhance-" + this.id + "-applied", true);
				$(this.elems).addClass("enhanced");
				$(this.elems).attr("data-enhance-" + this.id + "-applied", true);
				//$(this.elems).addClass("enhance-"+ this.id + "-applied");
				this.handler(target);
            /*} catch(e) {
				if (hasConsole){
					console.error("Enhancement failed: " + this.title);
					//IE hasConsole but not .dir function
            ...
            if (hasConsoleGroup) console.groupEnd();
            timeConsoleEnd(this.id, desc, this, target);
        });
        if (hasConsoleGroup) console.groupEnd();
        return this;
    }
